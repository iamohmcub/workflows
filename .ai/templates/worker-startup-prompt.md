...
9. Read `.ai/global/routing.matrix.yml`.
10. Read `.ai/global/agent.positions.yml`.
11. Read `.ai/global/parallel.delivery.yml`.
12. Read `.ai/workspace/workspace.yml`.
13. Read `.ai/workspace/stacks.yml`, `.ai/workspace/code-style.yml`, `.ai/workspace/project-structure.yml`, `.ai/workspace/tools.yml`, and `.ai/workspace/qa-process.yml`.
14. Follow the manifest `load_order`.
...
8. Use `.ai/global/routing.matrix.yml` to identify responsible, consulted, and informed roles.
9. Create an impact report if another role, repo, lane, API, data contract, environment, security boundary, or customer commitment is affected.
10. Create a handoff note before moving downstream.
11. Complete DoD item by item before handoff.
12. Commit completed work with the correct agent callsign from `.ai/global/agent.positions.yml`.
